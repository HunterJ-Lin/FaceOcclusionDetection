...
class DataSet:
    ...
    def __get_samples_num(self, train_label_file, val_label_file, test_label_file):
        ...
        assert os.path.exists(train_label_file) and os.path.exists(val_label_file) and os.path.exists(test_label_file)
    ...
    def load_batch_data_label(self, filename_list, label_list, label_num=1, shuffle=True, num_classes=6):
        ...
        for i in range(0, max_num, self.batch_size):
            ...
            if label_num == 1:
                batch_y = torch.nn.functional.one_hot(batch_y,num_classes=num_classes+1)
            else:
            ...
            if shuffle:
                ...
                batch_x = batch_x[idx]
                batch_y = batch_y[idx]
            yield torch.tensor(batch_x).float().permute(0,3,1,2), torch.tensor(batch_y).float()
...
class Cofw(DataSet):
    ...
    def data_generator(self, label_file_name='train.txt', shuffle=True):
