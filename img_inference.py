...
class ImageTester():
    def __init__(self, net=None, input_size=96, fine_tune=False, face_landmark_path='./models/shape_predictor_68_face_landmarks.dat'):
    ...
    def run(self,img_path):
        ...
        if len(face_rects) > 0:
            ...
            x = torch.tensor(input_img).float().unsqueeze(0).permute(0,3,1,2)
            pred = self.net(x)
        ...
        cv2.imwrite("detect_result.jpg", frame)
...
if __name__ == '__main__':
    net = models.FODNet(class_num = CLASS_NUM,
                        input_size = 96, 
                        fine_tune = False)
    state_dict = torch.load('output/model_final.pth',map_location=torch.device('cpu'))
    net.load_state_dict(state_dict)
    image_tester = ImageTester(net=net)
    image_tester.run('datasets/cofw/test/27.jpg')
